...
int solve(int row, int col, int board[][SIZE]) {
	...
	if (board[row][col] != 0) {
		if (find_next_rect(&new_row, &new_col, row, col) == 1) 
			return(1);										

		return(solve(new_row, new_col, board));
	}
	else {											
		for (num = 1; num <=9; num++) {				
			if (check_row(row, col, num, board) && 	
				check_col(row, col, num, board) &&  
				check_subrect(row, col, num, board)) {
				board[row][col] = num;
				if (find_next_rect(&new_row, &new_col, row, col) == 1) 	
					return(1);										   	
				if (solve(new_row, new_col, board) == 1)
					return(1);			
				else {					 
